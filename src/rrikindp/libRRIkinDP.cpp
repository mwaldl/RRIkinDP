...
PYBIND11_MODULE(libRRIkinDP, m) {
    ...
    py::class_<IntaRNA::RnaSequence>(
        m, "RnaSequence") // IntaRNA::RnaSequence s2(seq_b_id, seq_b);
        .def(py::init<const std::string, const std::string>());
    // Bind the Intarna Interaction class  // IntaRNA::Interaction
    // interaction(s1, s2); interaction.basePairs = bps_list_b0;
