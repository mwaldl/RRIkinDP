...
PYBIND11_MODULE(libRRIkinDP, m) {
    ...
    py::class_<EM>(m, "EM")
        //.def(py::init<IntaRNA::Interaction, std::string, std::string,
        // std::string, std::string, bool, bool, double>())
        .def(py::init<const IntaRNA::Interaction, const std::string &,
    ...
    // Bind IntaRNA RnaSequence class
    // IntaRNA::RnaSequence s2(seq_b_id, seq_b);
    py::class_<IntaRNA::RnaSequence>(m, "RnaSequence")
        .def(py::init<const std::string, const std::string>());
    // Bind the Intarna Interaction class
    ...
    // Bind IntaRNA IntaRNA Interaction::BasePair class
    py::class_<IntaRNA::Interaction::BasePair>(m, "BasePair")
        .def(py::init<const int, const int>())
        .def_readwrite("first", &IntaRNA::Interaction::BasePair::first)
        .def_readwrite("second", &IntaRNA::Interaction::BasePair::second);
}
