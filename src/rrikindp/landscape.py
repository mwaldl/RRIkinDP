...
class EnergyLandscape(rrikindp.EM):
    ...
    def get_states(self):
        ...
        for i in range(0, self.interaction_length):
            for j in range(i, self.interaction_length):
                ...
                ed1 = self.get_ED1(i, j) / 100
                ed2 = self.get_ED2(i, j) / 100
                e_hybrid = self.get_hybride_e(i,j)/100
                states.append([i, j, energy,ed1, ed2, e_hybrid])
        return states
    ...
    def save_states(self, states_file):
        with open(states_file, 'w') as out_file:
            ...
            for i in range(0, self.interaction_length):
                for j in range(i, self.interaction_length):
                    energy = self.get_e(i, j)
                    ed1 = self.get_ED1(i, j) 
                    ed2 = self.get_ED2(i, j) 
                    e_hybrid = self.get_hybride_e(i,j)
                    out_file.write(f"{i}\t{j}\t{energy}\t{ed1}\t{ed2}\t{e_hybrid}\n")
...
def plot_energy_landscape(
    ...
    figsize=(9, 7),  # figure size in inches, if None size is computed from matrix dimensions
    ...
    if path is not None:
        # start
        ax.add_patch(Rectangle((path[0][1], path[0][0]), 1, 1, fill=False, edgecolor='gold', lw=2))
        # draw path
        y, x = zip(*path)
        ax.plot([t+0.5 for t in x], [t+0.5 for t in y], color='grey', ls='--', lw=2)
        # end
        ax.add_patch(Rectangle((path[-1][1], path[-1][0]), 1, 1, fill=False, edgecolor='darkorange', lw=2))
    
    # draw input barrier state
    if barrier is not None:
        ax.add_patch(Rectangle((barrier[1], barrier[0]), 1, 1, fill=False, edgecolor='forestgreen', lw=2, ls=':'))
